...
function TodayReviewsWidgetComponent({ flashcards }: Props) {
  ...
  return (
    <div className="min-h-full relative overflow-hidden rounded-4xl bg-linear-to-br from-slate-900/90 via-slate-800/90 to-slate-900/90 border border-slate-700/50 backdrop-blur-2xl shadow-2xl">
      {/* Efeitos de fundo */}
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            <p className="font-medium text-lg text-white">Hoje</p>
          </div>
      ...
      <div className="relative p-5">
        {flashcards.length > 0 ? (
          <div
            className="space-y-2.5 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-slate-800/50 max-h-96"
            onClick={() => router.push(`/dashboard/review`)}
          >
            {flashcards.map((card, index) => {
              const diffStyle =
                difficultyConfig[card.difficulty as $Enums.FlashcardDifficulty];
