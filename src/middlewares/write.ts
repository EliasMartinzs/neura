 ...
 * Limita requisições de escrita a **40 por minuto**.
 */
...
/**
 * Middleware de segurança para requisições críticas de escrita (write).
 *
 *  Funcionalidades:
 * - Aplica **limite de taxa (rate limiting)** usando Arcjet.
 * - Limita usuários a **40 ações por minuto**.
 * - Retorna **HTTP 429** quando o usuário ultrapassa o limite de ações.
 * - Retorna **HTTP 403** quando negado por outros motivos.
 * - Calcula e retorna mensagem indicando **tempo restante até reset do limite**.
 *
 * ⚡ Uso:
 * `app.use(writeSecurityMiddleware)` em rotas sensíveis de escrita.
 *
 * @param c - Contexto do Hono, com acesso a `c.req` e `c.get("user")`.
 * @param next - Função para chamar o próximo middleware na stack.
 * @returns `JSON` com mensagem de erro (429 ou 403) ou chama `next()` se permitido.
 */
export const writeSecurityMiddleware = createMiddleware<{
